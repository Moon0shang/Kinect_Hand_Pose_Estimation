...
if __name__ == "__main__":
    dp = Depth()
    depth = np.load('./240.npy')

    depth = depth.clip(0, 4096).reshape(424, 512)
    cv2.imshow('depth1', np.uint8(depth/16))
    hand_contour = dp._find_contour(depth)
    darks = np.zeros((424, 512), dtype=np.uint8)
    if cv2.contourArea(hand_contour[0]) < 1000 or cv2.contourArea(hand_contour[0]) > 5000:
        dp.cover = np.uint8(depth/16.)
    else:
        seg_depth = dp._segment(depth, hand_contour, darks)
        # np.save('./seg_depth%d.npy' % i, seg_depth)
        # i += 1
    cv2.imshow("depth", dp.cover)
    cv2.imwrite('./normal.png', dp.cover)
    np.save('./normal.npy', seg_depth)
    cv2.waitKey(0)
